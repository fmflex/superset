...
def downgrade():
    ...
    for row in bind.execute(filter_tables):
        filters_copy_ids = []
        filter_query = rlsf.select().where(rlsf.c.id == row._mapping["rls_filter_id"])
        filter_params = dict(bind.execute(filter_query).mappings().fetchone())
        origin_id = filter_params.pop("id", None)
        table_ids = bind.execute(
            sa.select(rls_filter_tables.c.table_id).where(
                rls_filter_tables.c.rls_filter_id == row._mapping["rls_filter_id"]
            )
        ).fetchall()
        ...
        for role in bind.execute(roles_query):
            for copy_id in filters_copy_ids:
                role_filter = rls_filter_roles.insert().values(
                    role_id=role._mapping["role_id"], rls_filter_id=copy_id
                )
